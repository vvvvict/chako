...
<div class="<?php echo $pluralVar; ?> index">
	...
	<?php endforeach; ?>
		<th class="actions"><?php echo "<?php echo __('action'); ?>"; ?></th>
	</tr>
	</thead>
	<tbody>
	<?php
...
<div class="actions">
	<h3><?php echo "<?php echo __('操作'); ?>"; ?></h3>
	<ul>
		<li><?php echo "<?php echo \$this->Html->link(__('新規追加 " . $singularHumanName . "'), array('action' => 'add')); ?>"; ?></li>
<?php
	$done = array();
	foreach ($associations as $type => $data) {
		foreach ($data as $alias => $details) {
			if ($details['controller'] != $this->name && !in_array($details['controller'], $done)) {
				echo "\t\t<li><?php echo \$this->Html->link(__('リスト " . Inflector::humanize($details['controller']) . "'), array('controller' => '{$details['controller']}', 'action' => 'index')); ?> </
>\n";
				echo "\t\t<li><?php echo \$this->Html->link(__('新規追加 " . Inflector::humanize(Inflector::underscore($alias)) . "'), array('controller' => '{$details['controller']}', 'action' => 'add')); ?> </li>\n";
